...
public:
    ...
    //! Редактирование данных заметки с индексом \a index, заголовком \a title и записью \a text.
    void edit(QModelIndex index, QString title, QString text, qint32 icon);
    ...
    //! Сохраняет записную книжку
    void SaveAsText(QTextStream &ost) const;
    //! Удаляет заметку с индексом \a idx из записной книжки.
    void erase(SizeType idx);
    //! Возвращает адрес заметки по индексу \a index.
